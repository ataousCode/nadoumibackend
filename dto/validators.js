...
export const email = () =>
  Joi.string().email().lowercase().trim().messages({
...
export const otp = () =>
  Joi.string().length(6).pattern(/^\d+$/).messages({
    "string.length": "OTP must be 6 digits",
    "string.pattern.base": "OTP must contain only numbers",
    "string.empty": "OTP is required",
  });

export const name = (field = "Name", min = 1, max = 50) =>
  Joi.string()
    .trim()
    .min(min)
    .max(max)
    .messages({
      ...
      "string.min": `${field} must be at least ${min} character${
        min > 1 ? "s" : ""
      }`,
      "string.max": `${field} must not exceed ${max} characters`,
    });

export const uuid = (field = "ID") =>
  Joi.string()
    .uuid()
    .messages({
